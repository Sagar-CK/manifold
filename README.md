<p align="center">
  <img src="public/manifold-icon-128.png" alt="Manifold" width="128" height="128" />
</p>
...
```bash
pnpm install
pnpm setup:dev    # PDFium, FFmpeg, app icons
...
```
manifold/
├── config/           # Shared app config (folder exclude rules)
├── docs/             # Documentation and README assets
├── electron/         # Main process (IPC, indexing, embeddings, Qdrant)
├── public/           # Static assets served by Vite (icons, favicon)
├── resources/        # Runtime binaries (PDFium, FFmpeg, Qdrant) — populated by setup scripts
├── scripts/          # Dev, packaging, and binary setup scripts
└── src/              # Renderer (React UI, pages, hooks)
```
