...
- **FFmpeg / ffprobe** — video thumbnails in the main process (`electron/services/thumbnails.ts`)
...
- **PDF page thumbnails** — the thumbnail pipeline tries **`pdftoppm`** (Poppler) from `PATH` first, then falls back to `pdfjs-dist` + `@napi-rs/canvas`; install Poppler on developer machines if you want the preferred PDF preview path
...
These directories are copied into the app bundle via **electron-builder** `extraResources` (`package.json` → `build.extraResources`: `from: "resources"`, `to: "resources"`). At runtime the main process resolves binaries under the repo in development and under `process.resourcesPath/resources/<component>/` when packaged, with legacy fallback candidates under `process.resourcesPath/<component>/`.
