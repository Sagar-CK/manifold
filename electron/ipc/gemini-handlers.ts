...
import { judgeTag, readGeminiApiKeyOrThrow } from "../services/gemini-api.js";
import {
  clearGeminiApiKey,
  geminiApiKeyStatus,
  saveGeminiApiKey,
} from "../services/gemini-key-store.js";
import { MAX_EMBED_FILE_BYTES } from "../services/scan-walk.js";
...
export function registerGeminiHandlers(ctx: IpcContext): void {
  ipcMain.handle("gemini_judge_tag", async (_e, payload) => {
    ...
    const makePart = async (fp: string): Promise<unknown> => {
      ...
      if (st.size === 0) {
        return { text: `Empty file: ${path.basename(fp)}` };
      }
