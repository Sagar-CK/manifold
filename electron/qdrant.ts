...
import { qdrantBinaryCandidates, resolveFirstExisting } from "./app-paths.js";
...
function devDockerHint(): string {
  ...
  return " Start Qdrant with `pnpm qdrant:up` (Docker).";
}
...
function buildClient(restUrl: string): QdrantClient {
...
export async function qdrantStatus(): Promise<QdrantStatus> {
  ...
  const rest =
    runtime.restBaseUrl ??
    grpcUrlToRestBase(resolveQdrantEnvUrl() ?? grpc);
