...
import { repoRoot } from "../core/app-paths.js";
...
async function qdrantDockerQuickStartAvailable(): Promise<{
  available: boolean;
  reason?: string;
  ...
  if (!composeFile) {
    return {
      ...
      composeFile: null,
      reason:
        "Could not find docker-compose.yml. Run Manifold from the project folder so in-app setup can start Qdrant.",
    };
  }
...
export async function startQdrantDockerContainer(): Promise<{
  message: string;
}> {
  ...
  if (!availability.available || !availability.composeFile) {
    const reason =
      availability.reason ??
      "Could not start Qdrant with Docker. Install Docker Desktop, then use Set up Qdrant again.";
