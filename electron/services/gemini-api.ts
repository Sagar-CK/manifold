...
export const GEMINI_MODEL = "models/gemini-embedding-2";
export const GEMINI_OCR_MODEL = "models/gemini-3.5-flash";
const EMBED_TIMEOUT_MS = 120_000;
const OCR_TIMEOUT_MS = 180_000;
const MAX_ATTEMPTS = 5;
const GEMINI_EMBED_PDF_MAX_PAGES = 6;
const GEMINI_EMBED_PDF_CHUNK_PAGES = 6;
...
function inlineDataPart(
  ...
  if (bytes.byteLength === 0) {
    throw new Error(`Cannot submit empty ${mimeType} data to Gemini.`);
  }
...
async function embedMultimodalSingle(
  ...
  return geminiEmbedPost(apiKey, {
    content: {
      parts: [inlineDataPart(mimeType, bytes)],
    },
    output_dimensionality: OUTPUT_DIM,
  });
}
...
async function embedPdfBinaryChunks(
  ...
  const chunks = (
    await splitPdfBytes(bytes, GEMINI_EMBED_PDF_CHUNK_PAGES)
  ).filter((chunk) => chunk.byteLength > 0);
  if (chunks.length === 0) {
    throw new Error("PDF split produced no non-empty chunks for embedding");
  }
  const vectors: number[][] = [];
  for (const chunk of chunks) {
    vectors.push(await embedMultimodalSingle(apiKey, "application/pdf", chunk));
  }
  return averageEmbeddings(vectors);
}
...
async function embedPdfMultimodal(
  ...
  if (bytes.byteLength === 0) {
    throw new Error("Cannot submit empty PDF data to Gemini.");
  }
  ...
  try {
    ...
    // If page count fails, try extracted text / split fallbacks below.
  }

  if (pageCount !== null && pageCount <= GEMINI_EMBED_PDF_MAX_PAGES) {
    try {
  ...
  // Gemini Embedding 2 accepts PDFs up to 6 pages per request. For longer PDFs,
  // prefer extracted text; it is cheaper and avoids many binary chunk requests.
  try {
    return await embedPdfFromExtractedText(apiKey, bytes);
  } catch {
...
async function extractPdfTextWithGemini(
  ...
  try {
    ...
    const parts: string[] = [];
    for (const chunk of chunks.filter((c) => c.byteLength > 0)) {
