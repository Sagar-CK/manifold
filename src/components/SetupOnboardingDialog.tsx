...
function SetupStepIndicator({
  ...
  return (
    <div
      className="flex size-7 shrink-0 items-center justify-center rounded-full border border-border bg-muted/30 text-xs font-medium tabular-nums text-muted-foreground"
      aria-hidden
    >
      {index}
    </div>
...
export function SetupOnboardingDialog({
  ...
  const qdrantIssue = envIssues.find((issue) => issue.id === "qdrant-unreachable");
  ...
  const qdrantState: StepState = loadingStep === "qdrant"
    ? "loading"
    : qdrantComplete
      ? "complete"
      : "pending";
  ...
  async function handleStartQdrant() {
    ...
    const toastId = toast.loading("Setting up Qdrant in Docker…");
    try {
          ...
          <ol className="flex flex-col gap-5">
            ...
            <li>
              <SetupStepRow
                ...
                description="Add at least one include folder under Indexing."
                state={foldersState}
              >
                <Button type="button" size="sm" onClick={openSettings}>
                  Open Settings
