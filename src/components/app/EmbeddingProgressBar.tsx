        ...
        {showControls && embedding ? (
          <div className="flex items-center gap-1">
            {embeddingPhase === "paused" ? (
              ...
              >
                <HugeIcon icon={PauseIcon} className="h-4 w-4" aria-hidden />
              </Button>
            )}
            <Button
              variant="ghost"
              size="icon-sm"
              disabled={
                controlsDisabled || !onCancel || embeddingPhase === "cancelling"
              }
              aria-label="Cancel indexing"
              onClick={() => void onCancel?.()}
            >
              <HugeIcon icon={Cancel01Icon} className="h-4 w-4" aria-hidden />
            </Button>
          </div>
        ) : null}
      </div>
