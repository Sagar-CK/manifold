...
function normalizeExcludeSegment(value: string): string {
  return value
    .trim()
    .replace(/\\/g, "/")
    .replace(/^\/+|\/+$/g, "");
...
export function normalizeExcludeSegments(values: readonly string[]): string[] {
  const seen = new Set<string>();
  const out: string[] = [];
  for (const value of values) {
    const normalized = normalizeExcludeSegment(value);
...
export function createDefaultLocalConfig(): LocalConfig {
  return {
    ...
    defaultFolderExcludeSegments: normalizeExcludeSegments(
...
export function normalizeLocalConfig(
  ...
  return {
    ...
    defaultFolderExcludeSegments: normalizeExcludeSegments(
      parsed?.defaultFolderExcludeSegments ??
        defaults.defaultFolderExcludeSegments,
    ),
    extensions:
