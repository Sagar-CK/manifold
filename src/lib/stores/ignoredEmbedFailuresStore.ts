...
export function clearIgnoredEmbedFailurePath(path: string): void {
  ignoredEmbedFailuresStore.setSnapshot((prev) =>
    prev.includes(path) ? prev.filter((p) => p !== path) : prev,
  );
}

export function filterIgnoredEmbedFailures<T extends { path: string }>(
  failures: T[],
): T[] {
